...
# TASK 2
HORIZONTAL_CAMERASLACK = 180 # fix for TASK 1 width change
VERTICAL_CAMERASLACK = 90
...
def main():
    ...
    pygame.display.set_icon(pygame.image.load('projects\squirrelio\gameicon.png'))
    DISPLAYSURF = pygame.display.set_mode((WINWIDTH, WINHEIGHT))
    pygame.display.set_caption('Squirrel Eat Squirrel')
    BASICFONT = pygame.font.Font('freesansbold.ttf', 32)

    # load the image files
    L_SQUIR_IMG = pygame.image.load('projects\squirrelio\squirrel.png')
    R_SQUIR_IMG = pygame.transform.flip(L_SQUIR_IMG, True, False)
    GRASSIMAGES = []
    for i in range(1, 5):
        GRASSIMAGES.append(pygame.image.load('projects\squirrelio\grass%s.png' % i))
...
def makeNewSquirrel(camerax, cameray):
    ...
    if random.randint(1, 100) <= 33:
        sq['verticalMoveDirection'] = 'up' if random.randint(0, 1) == 0 else 'down'
        sq['movex'] = 0

        if sq['verticalMoveDirection'] == 'down':
            sq['movey'] = -getRandomVelocity()
        else:
            sq['movey'] = getRandomVelocity()
    else:
        sq['verticalMoveDirection'] = None
